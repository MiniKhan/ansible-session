Ok in this excersie I'll be using the Ad-Hoc Commads for example I have use the Ping command whihc is the ad hoac command . so it is basically single sinlge command.An Ansible ad hoc command uses the /usr/bin/ansible command-line tool to automate a single task on one or more managed nodes. ad hoc commands are quick and easy, but they are not reusable.
